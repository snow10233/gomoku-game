...
class AudioManager(QObject):
    def __init__(self):
        ...
        self.songs = {
        "menu": os.path.join(base_dir, "audio", "backgroundmusic", "menu.mp3"),
        "play": os.path.join(base_dir, "audio", "backgroundmusic", "game.mp3"),
        "victory": os.path.join(base_dir, "audio", "soundeffect", "victory.mp3"),
        "defeat": os.path.join(base_dir, "audio", "soundeffect", "fail.mp3")
        }

        print(f"載入選單音樂: {self.songs['menu']}")
        # 錯誤監控
        self.p1["player"].errorOccurred.connect(lambda e, m: print(f"BGM_P1 Error: {m}"))
        self.p2["player"].errorOccurred.connect(lambda e, m: print(f"BGM_P2 Error: {m}"))
